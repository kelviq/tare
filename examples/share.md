...
|---|---|
| API requests | 75,259 |
| Duplicate transcript entries collapsed | 64,783 |
| Sessions | 13,033 |
| Tool calls | 72,886 |
| Input tokens | 1,092,052 |
| Output tokens | 29,237,718 |
| Cache creation tokens | 321,719,353 |
| Cache read tokens | 2,413,284,525 |
...
|---|---|---|
| `claude-sonnet-4-6` | 68,085 | 44.8% |
| `claude-fable-5` * | 2,366 | 32.9% |
| `claude-opus-4-8` | 1,200 | 15.0% |
| `claude-sonnet-5` | 3,141 | 4.1% |
| `claude-opus-5` | 464 | 3.2% |
...
- **Some weight rests on a placeholder rate** — claude-fable-5 has no published price, so MODEL_RATES prices it as Opus. It is 2,366 requests (505.76M tokens) and shows up as 33% of weight, but that share is an artifact of the guess. Judge by requests and tokens until the rate is known.
...
- **Read is 74% of injected context** — 21648 calls, 105.84M tokens injected, 578.58M after re-sending. One noisy tool can dominate an entire week.

_Generated by ccaudit. Contains no prompts, file paths, file contents, command arguments, session ids or account identifiers._