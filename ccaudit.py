...
~/.claude/projects/<slug>/<session-id>.jsonl and answers three questions:
  ...
  3. Does anything look anomalous enough to be worth reporting as a bug?
...
No dependencies, no network, nothing leaves your machine. Python 3.9+.
  ...
  python3 ccaudit.py --bug-report bug.md    # redacted, shareable
...
def run_checks(requests, tools, stats, tz):
    ...
    if per_sess:
        ...
        if c > 400:
            out.append(("flag", f"One session made {c} API calls",
                        f"Session {s[:20]}… That volume usually means an agent "
                        "loop that didn't terminate. Read that transcript."))
...
def bug_report(requests, tools, stats, tz, days, path):
    ...
    for sev, head, detail in run_checks(requests, tools, stats, tz):
        if sev != "ok":
            L.append(f"- **{head}** — {detail}")
    L += ["", "_Generated by ccaudit. Contains no prompts, file paths, file "
          "contents, command arguments or account identifiers._"]
...
def main():
    ...
    if a.bug_report:
        bug_report(requests, tools, stats, a.tz, a.days, a.bug_report)
        print(f"Redacted bug report → {a.bug_report}")
